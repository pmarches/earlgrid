...
public class GrepCmdSpecificationTest {
  ...
  public void testPerformComputation() throws Exception {
    ...
    ExecutionHistoryRecord mockResult = client.requestCommandExecution("mock output 4 100");

    ExecutionHistoryRecord simpleGrep = client.requestCommandExecution("history 0 | grep cell-A-0.1");
    assertEquals("result was "+simpleGrep.getOut().toString(), 10, simpleGrep.getOut().getRowCount());

    ExecutionHistoryRecord invertedGrep = client.requestCommandExecution("history 0 | grep -v cell-A-0.1 | grep -v 'cell-.*-0.3'");
    assertEquals("result was "+invertedGrep.getOut().toString(), 80, invertedGrep.getOut().getRowCount());
    
    ExecutionHistoryRecord regularGrep = client.requestCommandExecution("history 0 | grep 'cell-A-0.1' | grep cell-.*-001");
